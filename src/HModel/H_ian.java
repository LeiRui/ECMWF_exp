...
public class H_ian {
    ...
    private int qck_r1;//范围查询的左端点在该列分布的位置百分比
    private int qck_r2;//范围查询的右端点在该列分布的位置百分比

    private int[] qack_p;//按照ACK顺序映射后,其余ckn-1个点查询值在该列分布的位置百分比，有占位
    ...
    public String getSql(String ks, String cf) {
        String q_format = "select * from " + ks + "." + cf + "  where name = 'FRACTION_OF_CLOUD_COVER'" +
                " and pkey=%d";
        for (int i = 0; i < ckn; i++) {
            if (i == qackn) {
    ...
    public BigDecimal calculate() {
        ...
        for (int i = 0; i < qackn; i++) { // qackn是从0开始的，这里刚好表示qckn-1个
            resP *= ACKdist.get(i).getPointP(qack_p[i]);
        }
        resP *= ACKdist.get(qackn).getRangeP(new RangePair(qck_r1, qck_r2));
